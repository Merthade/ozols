...
function render() {
    ...
    const level = getLevel(oaks);
    const progress = getProgress(oaks);
    ...
    // Debug panel display
    const debugOaks = document.getElementById('debug-oaks');
    if (debugOaks) debugOaks.textContent = oaks;
...
function updateOaks(delta, reason) {
    const prevLevel = getLevel(state.oaks);
    state.oaks = Math.max(0, state.oaks + delta);
    const newLevel = getLevel(state.oaks);
    render();
    if (reason) {
...
function activateVip(card, e) {
    ...
    if (wasActive) {
        ...
        updateOaks(-1000, 'VIP customer for a week');
    }
}
...
function updatePathHelperButtons() {
    ...
    const complete = document.getElementById('debug-complete-path');
    const uncomplete = document.getElementById('debug-uncomplete-path');
    ...
    complete.disabled = !hasIncomplete;
    uncomplete.disabled = !hasCompleted;
}
